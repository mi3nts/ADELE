...
# gets the number of words spoken
# input - the transcribed text
def num_of_words(text):
    return len(text.split(' ')) - 1 # for empty string at end

# gets the number of unique words spoken
# input - the transcribed text
def num_of_words_unique(text):
    words = text.split(' ')
    
    unique = list(set(words))

    return len(unique) - 1 # for empty string
...
def audio_to_text_output(audio_path, output_path):
    if os.path.exists(audio_path):
        text, temp = convert_audio_to_text(audio_path)
        ...
        text_file.write('Number of words: %d\n'%(num_of_words(temp)))
        text_file.write('Number of unique words: %d'%(num_of_words_unique(temp)))
        text_file.close()
    else:
